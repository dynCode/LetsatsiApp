...
(function(){
        ...
        */
            ...
            .success(function(data, status){
                ...
                if (data['error'] == 0) {
                    if (pageId === 2) {
                        ...
                        $scope.gmPage = data['html'];
                        myNavigator.pushPage('views/join/gm_page.html', { animation : 'slide' });
                    } else if (pageId === 5) {
                        $scope.bPage = data['html'];
                        myNavigator.pushPage('views/b_page.html', { animation : 'slide' });
                    } else if (pageId === 6) {
                        $scope.dmPage = data['html'];
                        myNavigator.pushPage('views/join/dm_page.html', { animation : 'slide' });
                    } else if (pageId === 7) {
                        $scope.alPage = data['html'];
                        myNavigator.pushPage('views/join/au_page.html', { animation : 'slide' });
                    } else if (pageId === 8) {
        ...
        $scope.LogIn = function() {
            ...
            if (user && pass) {
                ...
                $http.post(apiPath+'/app-results.php', {"reqType" : "login", "user" : user, "pass" : pass})
                .success(function(data, status){
                    if (data['error'] == 0) {
        ...
        $scope.pointsVoucher = function(partnerId) {
            ...
            $scope.data.errorCode = 'Processing, please wait...';
            $http.post(apiPath+'/app-results.php', {"reqType" : "PartnerVoucher", "partnerId" : partnerId, "cardNum" : $scope.MPacc})
            .success(function(data, status){
                modal.hide();
                console.log(data);
        ...
        $scope.discountVoucher = function(partnerId) {
            console.log(partnerId);
            $scope.data.errorCode = 'Processing, please wait...';
            $http.post(apiPath+'/app-results.php', {"reqType" : "DiscountPartnerVoucher", "partnerId" : partnerId, "cardNum" : $scope.MPacc})
            .success(function(data, status){
                modal.hide();
                console.log(data);
        ...
        $scope.contactMe = function() {
            ...
            if (contactName && contactSurname && contactCell && contactEmail) {
                modal.show();
                $scope.data.errorCode = 'Processing, please wait...';
                $http.post(apiPath+'/app-results.php', { "reqType" : "contactUs", "accountType" : contactAccount, "student" : contactStudent, "perks" : contactPerks, "comments" : contactComments, "cName" : contactName, "cSurname" : contactSurname, "cCell" : contactCell, "cEmail" : contactEmail })
                .success(function(data, status){
                    if (data['error'] == 0) {
                        modal.hide();
                        $scope.data.result = data['html'];
        ...
        $scope.getCouponCode = function() {
            modal.show();
            $scope.data.errorCode = 'Processing, please wait...';
            $http.post(apiPath+'/coupon-code.php', {"userRef" : $scope.MPacc})
            .success(function(data, status){
                modal.hide();
                console.log(data);
        ...
        $scope.MapPartnerVoucher = function(partnerId,partnerType) {
            ...
            $scope.data.errorCode = 'Processing, please wait...';
            $http.post(apiPath+'/app-results.php', {"reqType" : "MapPartnerVoucher", "partnerId" : partnerId, "cardNum" : $scope.MPacc, "partnerType": partnerType})
            .success(function(data, status){
                modal.hide();
                console.log(data);
    ...
    module.controller('mapController', function($scope, $http, $timeout, StreetView) {
        //API URL path
        var apiPath = 'http://www.myitmanager.co.za/letsatsi-app/api';
        ...
        $timeout(function(){
            ...
            $scope.$on('mapInitialized', function(event, evtMap) {
                map = evtMap;
                $scope.map = map;
                $scope.myLat = $scope.map.center.lat();
                $scope.myLng = $scope.map.center.lng();

                //console.log($scope.map);

                console.log("lat:" + $scope.myLat + " lng:" + $scope.myLng + " radius:" + $scope.mapRadius + " type:" + $scope.partnerType);

                $http.post(apiPath+'/pointsPartnerMapList.php', {"lat" : $scope.myLat, "lng" : $scope.myLng, "radius" : $scope.mapRadius, "type" : $scope.partnerType, "cat" : "%"})
...
document.addEventListener("DOMContentLoaded", function(event) { 
    ...
    if (document.documentMode || /Edge/.test(navigator.userAgent) || /MSIE/.test(navigator.userAgent) || /Trident/.test(navigator.userAgent)) {
        alert('This website is not supported for this browser, please make use of Google Chrome or view the site on your mobile device.');
    }
});